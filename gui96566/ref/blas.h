...
struct vec_u64x2
{
	...
	vec_u64x2()
	{
		for (unsigned i = 0; i < _num_limb; i++)
			v[i] = 0;
	}
	explicit vec_u64x2(const uint8_t *inp)
	{
		for (unsigned i = 0; i < _num_limb - 1; i++)
			v[i] = ((const uint64_t *)inp)[i];
		for (unsigned i = _idx_last_limb * 8; i < _num_byte; i++)
			((uint8_t *)v)[i] = inp[i];
		v[_idx_last_limb] &= _high_mask;
	}
	vec_u64x2(const vec_t &a)
	{
		for (unsigned i = 0; i < _num_limb; i++)
			v[i] = a.v[i];
	}

	explicit vec_u64x2(bool b)
	{
		uint64_t b0 = b;
		uint64_t b1 = 0;
		b1 -= b0;
	...
	void fdump(FILE *fp) const
	{
		...
		fprintf(fp, "[%3d][", W);
		for (int i = _idx_last_limb; i >= 0; i--)
	...
	uint64_t tail() const
	{
		if (W > 128)
		{
			uint64_t t = (v[2] << 16) | (v[1] >> 48);
			return t >> (W - 112 - tail_bit);
		}
		if (W > 64)
			return v[1] >> (W - 64 - tail_bit);
	...
	bool operator==(const vec_t &a) const
	{
		vec_t t = a;
		t ^= *this;
		return t.is_zero();
...
struct mat_u64
{
	...
	static void mul(mat_u64<L_VEC, L_VEC> &c, const mat_u64<N_VEC, L_VEC> &a, const mat_u64<L_VEC, N_VEC> &b)
	{
		for (unsigned k = 0; k < L_VEC; k++)
		{
			c.v[k].set_zero();
			for (unsigned i = 0; i < N_VEC; i++)
				c.v[k] ^= b.v[k].get_ele(i) ? a.v[i] : vec_v::zero();
		}
	}
};
	...
	for (unsigned i = 0; i < W; i++)
	{
		if (!a.v[i].get_ele(i))
		{
			for (unsigned j = i + 1; j < W; j++)
			{
				if (!a.v[j].get_ele(i))
					continue;
				typename mat_u64<W, W>::vec_h tt = a.v[i];
				a.v[i] = a.v[j];
				a.v[j] = tt;
				tt = b.v[i];
				b.v[i] = b.v[j];
